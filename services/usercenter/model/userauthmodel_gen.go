...
type (
	...
	UserAuth struct {
		...
		UserId     int64     `db:"user_id"`   // local 用户 id，对应 user 表中的 id
		AuthKey    string    `db:"auth_key"`  // 第三方平台唯一用户 id
...
func (m *defaultUserAuthModel) Update(ctx context.Context, newData *UserAuth) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.DelState, newData.Version, newData.UserId, newData.AuthKey, newData.AuthType, newData.Id)
	}, userAuthAuthTypeAuthKeyKey, userAuthIdKey, userAuthUserIdAuthTypeKey)
	return err
}
